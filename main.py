    ...
    try:
        while target_spamming.get(ctx.channel.id):
            for base in base_templates:
                ...
                # MAXIMUM EMOJIS / FILLER
                if fillermore_emojis:
                    emoji = fillermore_emojis[0]
                    max_repeat = (1990 - len(base)) // len(emoji)
                    msg = (emoji * max(1, max_repeat)) + " " + base + " 😈💢"
                elif spam_filter:
                    max_repeat = (1990 - len(base)) // len(spam_filter)
                    msg = (spam_filter * max(1, max_repeat)) + " " + base + " 😈💢"
                else:
                    msg = base + " 😈💢"
