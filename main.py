...
# Check DISCORD_TOKEN_1, DISCORD_TOKEN_2, ... automatically
i = 1
while True:
    token_key = f"DISCORD_TOKEN_{i}"
    token_value = os.environ.get(token_key)
    if not token_value:
        break
    tokens.append((token_key, token_value))
    i += 1

if not tokens:
    print("\x1b[38;5;196m[ERROR]: No token found!\x1b[0m")
    exit(1)

print(Fore.GREEN + f"[SUCCESS] Loaded {len(tokens)} token(s)" + Fore.RESET)
for name, t in tokens:
    print(Fore.CYAN + f"   • {name}" + Fore.RESET)

token = tokens[0][1]   # Uses first token
...
async def sudo(ctx, action: str = None, user: discord.User = None):
    ...
    # ==================== OWNER ID CHECK FROM CONFIG ====================
    owner_id = config.get("owner_id")
    if not owner_id or ctx.author.id != int(owner_id):
        await ctx.send("> **❌ Only the Owner (defined in config) can use this command.**", delete_after=5)
        return
    # ============================================================
