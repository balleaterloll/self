...
if not tokens:
    print("\x1b[38;5;196m[ERROR] No tokens found in config.json!\x1b[0m")
    print("Add tokens in config/config.json under 'tokens' list.")
    exit(1)

print(Fore.GREEN + f"[SUCCESS] Loaded {len(tokens)} tokens from config.json" + Fore.RESET)

# Use first token by default (you can change later)
token = tokens[0]
print(Fore.YELLOW + f"[RUNNING] Using Token 1 of {len(tokens)}" + Fore.RESET)
