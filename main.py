...
async def targetspam(ctx, *, target: str = None):
    ...
    await ctx.send(f"> **ULTRA FAST Target Spam Started** for `{target}`", delete_after=5)

    templates = [
        f"{target} BHOSDIKE 😈💢", f"{target} MADARCHOD 😈💢", f"{target} TMKC 😈💢",
        f"{target} TERI MAA KI CHUT 😈💢", f"{target} RO RNDYKE 😈💢", f"{target} GOJO ON TOP 😈💢"
    ]
...
async def stopgcnc(ctx):
    ...
    await ctx.send("> **✅ Stopped GC Name & Target NC loops.**", delete_after=5)
